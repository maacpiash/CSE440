    ...
    def getValue(self, sample):
        if sample.attributes[self.featID] < self.threshold:
            if type(self.left_child) is Node:
                ...
                try:
                    d = dict.fromkeys(self.left_child)
                    for i in range(global_classes_count):
                        d[i] = global_classes_list[i]
                    return d
                except:
            ...
            if type(self.right_child) is Node:
                ...
                try:
                    d = dict.fromkeys(self.right_child)
                    for i in range(global_classes_count):
                        d[i] = global_classes_list[i]
                    return d
                except:
...
for tree in global_forest:
    ...
    for sample in testers:
        vector_dict = tree.getValue(sample)
        try:
            m = max(vector_dict.keys())
            prediction = vector_dict[m]
            count = 0
            ...
            prediction = vector_dict
            # in case of homogeneous examples, DTL returns the class
            accuracy = 1
